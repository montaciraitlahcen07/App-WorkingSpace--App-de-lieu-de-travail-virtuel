...
void Authentification(HWND ULogin,HWND PLogin,FILE *UserData_2,HBRUSH creme,RECT WindowSize,HDC Mdc,HWND HandleWnd,SndTrd *SendingTools,CntTrd ConnectingTools,bool *Green,bool *safety)
{
    ...
    int usernameResult = GetWindowText(ULogin,username,sizeof(username));
    int passwordResult = GetWindowText(PLogin,password,sizeof(password));
    // check if bars of username and password are filled before they have been sent
